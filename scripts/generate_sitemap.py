#!/usr/bin/env python3
"""Generate sitemap.xml from root HTML pages and sync robots.txt."""
...
def iter_pages() -> Iterable[Path]:
    pages = sorted(ROOT.glob("*.html"), key=lambda p: order_key(p.name))
    return [p for p in pages if not p.name.startswith("__")]
...
def main() -> None:
    ...
    parser.add_argument(
        "--base-url",
        default="https://umkanelson.github.io/EleonorLab",
